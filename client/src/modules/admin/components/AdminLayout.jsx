...
const NAV = [
  ...
  { to: '/admin/footer',    label: 'Footer',
    icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5M12 17.25h8.25" /></svg> },

  { to: '/admin/shirt-config', label: 'Shirt Config',
  ...
  { to: '/admin/about',     label: 'About Page',
    icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M11.25 11.25l.041-.02a.75.75 0 011.063.852l-.708 2.836a.75.75 0 001.063.853l.041-.021M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9-3.75h.008v.008H12V8.25z" /></svg> },

  { to: '/admin/contact',   label: 'Contact Page',
    icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" /></svg> },

  { to: '/admin/privacy',   label: 'Privacy Policy',
...
const NavItem = ({ item, onClick }) => {
  ...
  if (item.divider) {
    return (
      <div className="px-3 pt-4 pb-1">
        <p className="text-[9px] font-medium tracking-widest uppercase text-white/20">{item.label}</p>
      </div>
    );
  }
  ...
  return (
    <Link to={item.to} onClick={onClick}
      ...
      style={{
        background: active ? 'rgba(107,66,38,0.18)' : 'transparent',
        color:      active ? '#fff' : 'rgba(255,255,255,0.45)',
        border:     active ? '1px solid rgba(107,66,38,0.25)' : '1px solid transparent',
      }}>
      <span style={{ color: active ? '#8B5A3C' : 'rgba(255,255,255,0.3)' }}>{item.icon}</span>
      {item.label}
    </Link>
  );
};
  ...
  const Sidebar = ({ onLinkClick }) => (
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-t border-white/[0.08]">
        ...
        <Link to="/" className="block text-xs text-white/30 hover:text-white/60 transition-colors mb-1">← Back to site</Link>
        <button onClick={handleLogout} className="text-xs text-red-400/50 hover:text-red-400 transition-colors">Logout</button>
      </div>
    </div>
  );

  return (
    <div className="min-h-screen flex bg-ink-black">
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex lg:flex-col w-56 flex-shrink-0 border-r border-white/[0.08]"
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed inset-y-0 left-0 w-60 z-50 flex flex-col border-r border-white/[0.08] lg:hidden"
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Mobile topbar */}
        <div className="lg:hidden flex items-center justify-between px-4 py-3 border-b border-white/[0.08] sticky top-0 z-30"
